...
class PublicData:
    ...
    def fetch_ohlcv(self, symbol):
        ...
        uri = f"/futures/udf/history?symbol={symbol.upper()}&resolution={res}&from={from_ts}&to={now}&countback={HISTORY_BARS}"
        try:
            ...
            df = pd.DataFrame({'open': o, 'high': h, 'low': l, 'close': c}, index=pd.to_datetime(t, unit='s', utc=True))
...
class PrivateExchange:
    ...
    def create_order(self, symbol, side, capital, leverage):
        # ساده‌سازی: در این نسخه فقط پیام می‌فرستیم
        send_telegram(f"سفارش {symbol} {side} با اهرم {leverage}")
